# v4.5.4 Explainer — Slides 1-50

**Deck**: `docs/menopause_sns_gtex_enhanced_2026-05-03_v4.5.4.pptx` (50 slides, 13.33 × 7.50 in widescreen, dated 2026-05-03)
**Predecessors**: v4.5.1 (2026-04-22) → v4.5.2 (2026-05-01) → v4.5.3 (2026-05-01) → v4.5.4 (2026-05-03). v4.5.3 was the post-audit clean version of the 37-slide deck. v4.5.4 adds 13 new analysis slides triggered by the 2026-05-03 collaborator-meeting feedback: ADRB3 mirror analyses for every ADRB2-only slide; a 50s-excluded sensitivity stratification; a menopause-signature-score-based stratification (Section 4); and a HALLMARK-vs-cytokine inflammation methods clarification. All new slides carry significance markers (`*` p<0.05, `**` p<0.01, `***` p<0.001) on group comparisons. The audit chain remains intact: 98 + 22 numerical claims still verified from rounds 1-2; the new analyses live in `tables/next_steps_2026-05-03_v4.5.4/` with their own audit log.
**Version lineage**:
...
**Interpretation**: the ADRB2-fibrosis effect is robust and intensifies when 50s are removed (older group becomes more truly post-menopausal). The apparent ADRB3-fibrosis correlation in the broad analysis is **driven entirely by the 50s decade** — when you remove those women, ADRB3-fibrosis is essentially zero in 60-79 women but is r=-0.36 specifically in 50-59 women.

**Mechanistic reading**: ADRB3 is acutely impaired during the menopausal transition (50s) but recovers afterward; ADRB2 impairment is lasting. This may reflect different desensitization dynamics for the two receptors.
...
### 0.F v4.5.4 Slide 31 — Q1 mirror: Fisher-z young-vs-older for ADRB3 (3 stratifications)

Text-based summary of the ADRB3 Fisher-z comparison across all three stratifications. Headline is the contrast between the 50s-included broad analysis (where ADRB3-fibrosis looks weakly negative), the peri-window (where it is r=-0.36 in 50-59 women), and the 50s-excluded broad analysis (where it is essentially zero). This slide presents the case described in §0.E in tabular form.
...
### 0.G v4.5.4 Slides 35, 37, 39, 41 — ADRB3 Saltiel-style scatter panels (4 mirrors)

Each slide mirrors its v4.5.3 ADRB2 counterpart (Slides 29-32 of v4.5.3 → Slides 34, 36, 38, 40 in v4.5.4) with ADRB3 substituted on the x-axis. The four panels per slide show ADRB3 vs Age / CCL2 / Fibrosis / Senescence. Asterisk markers in panel titles. Caption shows n, slope, R², p for each panel.

**Headline R² values** (v4.5.4 Slide 35, female subq, n=233):

| Endpoint | ADRB2 R² (Slide 34) | ADRB3 R² (Slide 35) |
|---|---:|---:|
| Age | 0.021 | **0.047** *** (ADRB3 stronger) |
| CCL2 | **0.090** *** (ADRB2 stronger) | 0.027 * |
| Fibrosis | **0.174** *** (ADRB2 dominant) | 0.026 * |
| Senescence | 0.076 *** | **0.093** *** (ADRB3 slightly stronger) |

**Interpretation**: ADRB2 is more strongly fibrosis- and inflammation-associated; ADRB3 is more strongly age- and senescence-associated. Both decline with age but they carry different downstream-correlation profiles.
...
**Bottom-line text on the slide**: *"ADRB3 is more strongly age- and senescence-associated; ADRB2 is more strongly fibrosis- and inflammation-associated. Both decline with age; the receptors carry different downstream-correlation profiles."*
...
- **senMayoLike v2** (`references/menopause_signature_senMayoLike_v2.tsv`, 30 genes with UP/DOWN direction): score = mean[z(UP genes)] − mean[z(DOWN genes)]. Signed — high = "more post-menopause-like".
- **curated v1** (`references/menopause_signature_curated.tsv`, 70 genes, no direction): score = mean[z(all genes)]. Unsigned magnitude.
...
**Concordance with age-bin grouping is partial**: pre / peri / post overlap substantially in score space. Many "pre" samples (by age) score in the high-tertile of senMayoLike, and many "post" samples score in the low-tertile. This justifies score-based stratification as a complementary rather than redundant grouping.
...
### 2026-05-01 Round-2 review corrections (this deck)

- Patch script: `scripts/make_pptx_2026-05-01_v4.5.3_round2_corrections.py`
- This explainer: `docs/menopause_sns_gtex_enhanced_2026-05-01_v4.5.3_explainer.md`
- Deck: `docs/menopause_sns_gtex_enhanced_2026-05-01_v4.5.3.pptx`
...
1. **The 50s decade carries the ADRB3-fibrosis signal** (Slide 29 sensitivity). When the 50s are removed from the older group (60-79 vs 20-49), the ADRB3-fibrosis correlation in older subq women drops from r=-0.17 to r=-0.06. The same removal makes the ADRB2-fibrosis correlation STRONGER (-0.43 → -0.48). This implies the two β-receptors track different time-courses through the menopausal transition.

2. **ADRB2 vs ADRB3 phenotype profiles diverge** (Slide 43 summary). In female subq, ADRB2 dominates fibrosis (R²=0.174 vs 0.026) and inflammation (CCL2 R²=0.090 vs 0.027), while ADRB3 leads on age (R²=0.047 vs 0.021) and senescence (R²=0.093 vs 0.076). The two receptors are NOT redundant readouts of a single menopause effect.

3. **Score-tertile stratification differs from age-bin stratification** (Slide 47). When females are grouped by their senMayoLike-v2 transcriptional menopause score rather than by their GTEx age bin, the pre/peri/post groups overlap substantially. About 20-25% of "pre" samples by age have high transcriptional scores (T3) and conversely. This is preliminary evidence that age-bin grouping under-resolves transcriptional state and supports the use of score-based grouping in the manuscript as a sensitivity analysis or primary stratification.
