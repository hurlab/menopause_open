# v4.6.1 Explainer — Slides 1-52

**Deck**: `docs/menopause_sns_gtex_enhanced_2026-05-07_v4.6.1.pptx` (52 slides, 13.33 × 7.50 in widescreen, dated 2026-05-07)

**v4.6.1 changelog (Section-4 score-vs-canonical-group alignment; deck grows from 50 → 52 slides)**:
- **New analysis** (`R/next_steps_2026-05-07_score_vs_menopause_group_alignment.R`): quantifies how each menopause-signature score (senMayoLike v2, curated v1) aligns with the canonical pre/peri/post grouping per depot. Reports Kruskal-Wallis, Spearman ρ vs ordinal stage, Jonckheere-Terpstra trend p, weighted Cohen's kappa, Adjusted Rand Index, and BH-adjusted pairwise Wilcoxon.
- **Headline numbers** (females): in subcutaneous, both scores show monotonic pre→peri→post increase (senMayo ρ=0.343, KW p=9.1e-8; curated ρ=0.319, KW p=5.4e-6) but the signal is essentially "pre vs not-pre" — peri vs post is NOT significant after BH correction in either score (senMayo BH p=0.51; curated BH p=0.059). In visceral, alignment is weak: senMayo marginal (ρ=0.18, KW p=0.049); curated NOT significant (KW p=0.30, ARI ≈ 0).
- **Weighted Cohen's kappa** (tertile vs canonical group): subq senMayo 0.226, subq curated 0.206, visc senMayo 0.111, visc curated 0.086 — fair-to-poor agreement. This is the parent observation that explained the earlier Slide 47 finding that the two scores' tertiles had kappa = 0.10–0.17 between *each other*: each score itself only weakly tracks the canonical grouping, so disagreement between the two scores' tertiles was the inherited consequence.
- **Practical implication**: keep pre/peri/post (or 20-49 vs 50-79) as the primary stratification; score-tertile remains supplementary, not a competing primary classifier.
- **Slide changes**:
  - NEW Slide 48: violin/box of each score by pre/peri/post per depot with on-figure stats.
  - NEW Slide 49: 3×3 contingency mosaic of score tertile × canonical group per (depot, score).
  - Old Slide 48 (ADRB2 score-tertile correlations) → renumbered to 50.
  - Old Slide 49 (ADRB3 score-tertile correlations) → renumbered to 51.
  - Old Slide 50 (Caveats and proposed next steps) → renumbered to 52.
- **Footers**: all 52 slides bumped v4.6.0 → v4.6.1.
- **Outputs on disk**: `tables/next_steps_2026-05-07_score_alignment/{score_vs_menopause_group_summary,score_vs_menopause_group_per_group_stats,score_tertile_vs_menopause_group_confusion,pairwise_wilcoxon_BH}.tsv`; `figs/next_steps_2026-05-07_score_alignment/{score_by_menopause_group_violin,score_tertile_vs_menopause_group_mosaic}.png`.
- **Patcher**: `scripts/make_pptx_2026-05-07_v4.6.1_score_alignment.py` (builds from v4.6.0; idempotent).
- **Verification**: 0 overflow, 0 text-pic occlusion, 52/52 footers say v4.6.1, 0/52 still say v4.6.0.
